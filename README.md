...
## Quick Start
For a quick start sample file, see the [sample](sample) source directory. If using a local copy of pyxmatters, just set the `xmatters` directory alongside the `config.py` and `sample.py` in the `/sample` directory.
